...
// public quote view /q/[token] and the hosted form pages: server-rendered, the
...
//
// It lives here, apart from the route, because two routes render it: the page
// a customer opens directly at `/b/<tenant>/<type>`, and the iframe document
// `/b/e/<tenant>/<type>` that `public/b.js` embeds in the tenant's own site
// (plan-booking.md §6.2). Embedding must not mean a second copy of the
// booking form that drifts from the first — the only difference between them
// is the wrapper, below.
...
export async function BookingView({
  ...
  tenantSlug: string;
  typeSlug: string;
  /** Inside an iframe on somebody's site: no page margins, no centring. */
  embedded?: boolean;
}) {

  const ip = clientIp(await headers());
  if ((await checkRateLimit(`booking-page:${ip}`, 60, 60_000)).limited) {
    ...
    <div
      className={
        embedded
          ...
          : "mx-auto flex max-w-2xl flex-col gap-6 p-6"
      }
    >
